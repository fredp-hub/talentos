...
import { generateGapAnalysis } from '@/lib/anthropic'
...
export async function POST(req: NextRequest) {
  try {
    ...
    const profile = [
      ...
      candidate.availability_date ? `Available: ${candidate.availability_date}` : null,
    ...
    const gap_analysis = await generateGapAnalysis(jd, profile)

    await supabase
      .from('ai_matches')
      .upsert(
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        { candidate_id, requisition_id, gap_analysis, generated_at: new Date().toISOString() } as any,
        { onConflict: 'candidate_id,requisition_id' }
      )

    return NextResponse.json({ gap_analysis })
  } catch (err) {
    console.error('[ai/gaps]', err)
